...
public class Util {
	...
	public static final String[] geocachingQueryParam = new String[] { "q" };
	...
	public static String degreesToMinutes(double degrees) {
		return String.format("%1$d %2$06.3f", (int) degrees, 60.0 * Math.abs(degrees
				- (int) degrees));
	}
	...
	public static String[] getLatLonFromQuery(final String uri) {
		...
		if (m.matches()) {
			return new String[] { cleanCoordinate(m.group(1)), cleanCoordinate(m.group(2)),
					m.group(3) };
		}
		m = Pattern.compile(CACHE_AT_BEGINNING).matcher(uri);
		m.matches();
		return new String[] { cleanCoordinate(m.group(2)), cleanCoordinate(m.group(3)), m.group(1) };
	}

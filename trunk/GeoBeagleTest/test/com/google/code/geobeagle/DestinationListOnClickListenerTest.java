...
public class DestinationListOnClickListenerTest extends TestCase {
	...
	public void testOnClick() {
		...
		final CharSequence[] locations = new CharSequence[] { "37 122 etc",
				"37 122 foo" };

		expect(dialogBuilder.setTitle(R.string.select_destination)).andReturn(
				dialogBuilder);
		expect(
				dialogBuilder.setItems(aryEq(new CharSequence[] {
						DestinationListOnClickListener.MY_LOCATION, "OAK",
						"SFO" }), (OnClickListener) eq(mockListener)))
				.andReturn(dialogBuilder);
		...
		DestinationListOnClickListener destinationListOnClickListener = new DestinationListOnClickListener(
				Arrays.asList(descriptions), Arrays.asList(locations),
				locationSetter, dialogBuilder) {
			protected OnClickListener createDestinationListDialogOnClickListener(
					List<CharSequence> previousLocations) {
				assertEquals(Arrays.asList(null, locations[1], locations[0]),
						previousLocations);
				return mockListener;
			}
		};
	...
	public void testDestinationListDialogOnClickListener() {
		LocationSetter locationSetter = createMock(LocationSetter.class);
		CharSequence[] locations = new CharSequence[] { null, "37 122 foo",
				"37 122 etc", };
